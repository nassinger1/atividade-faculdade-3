...
import { listEnrollments, deleteEnrollment } from "../../api/enrollments";
...
import { formatDate, accessStatus } from "../../hooks/dates";
...
export function EnrollmentList() {
  ...
  async function load() {
    ...
    try {
      ...
      setError(getErrorMessage(err, "Não foi possível listar as matrículas."));
    } finally {
      setLoading(false);
    }
  }
                ...
                {filtered.map((e) => {
                  ...
                  return (
                    <tr key={e.id}>
                      ...
                      <td className={styles.num}>{formatDate(e.startDate)}</td>
                      <td className={styles.num}>{formatDate(e.endDate)}</td>
                      <td>
                        <span
                          className={`${ui.badge} ${
